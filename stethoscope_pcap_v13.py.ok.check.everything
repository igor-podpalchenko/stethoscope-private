...
class SessionState:
    ...
    fin_c2s_seen: bool = False
    fin_s2c_seen: bool = False
    fin_c2s_end_seq: Optional[int] = None
    fin_s2c_end_seq: Optional[int] = None
    fin_c2s_notified: bool = False
    fin_s2c_notified: bool = False
...
class ReassemblyWorker(threading.Thread):
    ...
    def run(self) -> None:
        while not self._stop.is_set():
            ...
            st.touch(ts)
            if p.flags & 0x04:  # RST
                # RST tears down both directions immediately.
            ...
            # FIN is directional. We record the stream end (seq after last payload byte) and only
            # emit half-close once reassembly has drained up to that point (reordering can deliver
            # FIN before some earlier bytes).
            if p.flags & 0x01:  # FIN
                fin_end = p.seq + len(p.payload)
                if p.from_local:
                    st.fin_c2s_seen = True
                    st.fin_c2s_end_seq = fin_end
                else:
                    st.fin_s2c_seen = True
                    st.fin_s2c_end_seq = fin_end

            # Emit half-close events only when the corresponding direction is fully flushed.
            if st.fin_c2s_seen and not st.fin_c2s_notified and st.fin_c2s_end_seq is not None:
                ns = st.c2s.reasm.next_seq
                if ns is not None and ns >= st.fin_c2s_end_seq:
                    st.fin_c2s_notified = True
                    self._emit_chunk(ForwardChunk(
                        flow=st.flow,
                        ts=ts,
                        direction="c2s",
            ...
            if st.fin_s2c_seen and not st.fin_s2c_notified and st.fin_s2c_end_seq is not None:
                ns = st.s2c.reasm.next_seq
                if ns is not None and ns >= st.fin_s2c_end_seq:
                    st.fin_s2c_notified = True
                    self._emit_chunk(ForwardChunk(
                        flow=st.flow,
                        ts=ts,
                        direction="s2c",
            ...
            # Close session only after both directions have FIN and both have been drained.
            if st.fin_c2s_seen and st.fin_s2c_seen and st.fin_c2s_notified and st.fin_s2c_notified:
...
class SessionOutputs:
    ...
    connector_tasks: Dict[str, asyncio.Task] = field(default_factory=dict)
...
class OutputManager:
    ...
    async def ensure_session(self, flow: FlowKey) -> SessionOutputs:
        async with self._lock:
            ...
            if self.connector_enabled and self.conn_req_port > 0 and self.conn_resp_port > 0:
                t_req = asyncio.create_task(self._connector_loop(so, "requests"), name=f"connector:{flow.session_id}:requests")
                t_resp = asyncio.create_task(self._connector_loop(so, "responses"), name=f"connector:{flow.session_id}:responses")
                so.connector_tasks["requests"] = t_req
                so.connector_tasks["responses"] = t_resp
                so.tasks.extend([t_req, t_resp])

            return so

    async def _close_stream_outputs(self, so: SessionOutputs, stream: str, *, reason: str, cause: str) -> None:
        # Stop (or prevent) reconnect loops for this stream.
        t = so.connector_tasks.pop(stream, None)
        if t:
            t.cancel()

        # Close any active listener sink (if present). The per-connection handler will emit listener_disconnected.
        lw = so.listeners.get(stream)
        if lw:
            try:
                lw.writer.close()
            except Exception:
        ...
        # Close any active connector and emit connector_disconnected if it was connected.
        st = so.connector_state.get(stream)
        was_connected = bool(st and st.currently_connected)
        ...
        if st and st.currently_connected:
    ...
    async def close_stream(self, flow: FlowKey, *, stream: str, reason: str, cause: str = "input_close") -> None:
        async with self._lock:
            so = self.sessions.get(flow)
        if not so:
            return
        if stream not in ("requests", "responses"):
            return
        await self._close_stream_outputs(so, stream, reason=reason, cause=cause)
    ...
    async def close_session(self, flow: FlowKey, reason: str) -> None:
        ...
        # Close per-stream outputs first so we can emit proper disconnect events
        # even if we subsequently cancel the connector loop tasks.
        for stream in ("requests", "responses"):
            try:
                await self._close_stream_outputs(so, stream, reason=reason, cause="session_close")
            except Exception:
                pass
...
class Service:
    def __init__(self, cfg: Dict[str, Any], log: logging.Logger) -> None:
        ...
        # Per-direction startup buffering:
        # - Backward compatible with legacy 'buffer_bytes' (applies to both directions)
        # - New keys: c2s_buffer_bytes, s2c_buffer_bytes
        # - 0 disables buffering for that direction
        base_startup_buf = clamp_int(
            cap.get("buffer_bytes", default_startup_buf),
            default=default_startup_buf,
            lo=0,
            hi=max_startup_buf,
        )
        self.c2s_buffer_bytes = clamp_int(
            cap.get("c2s_buffer_bytes", base_startup_buf),
            default=base_startup_buf,
            lo=0,
            hi=max_startup_buf,
        )
        self.s2c_buffer_bytes = clamp_int(
            cap.get("s2c_buffer_bytes", base_startup_buf),
            default=base_startup_buf,
            lo=0,
            hi=max_startup_buf,
        )
        # Retained for convenience checks (any direction enabled)
        self.startup_buffer_bytes = max(self.c2s_buffer_bytes, self.s2c_buffer_bytes)
        ...
        # Aggregated (all-time) drop breakdown that survives session cleanup
        self._drop_count_agg: DefaultDict[Tuple[str, str], int] = defaultdict(int)  # (stream, reason) -> count
        self._drop_bytes_agg: DefaultDict[Tuple[str, str], int] = defaultdict(int)  # (stream, reason) -> bytes
    ...
    def drop_detail_snapshot(self, top_n: int = 16) -> Dict[str, Any]:
        """Aggregate drop counters across all sessions (all-time since process start).
        ...
        """
        # Prefer aggregated counters (survive session cleanup). Fall back to per-session map if needed.
        agg_count = getattr(self, "_drop_count_agg", None)
        agg_bytes = getattr(self, "_drop_bytes_agg", None)

        if not agg_count or not agg_bytes:
            # Legacy fallback
            if not self._drop_count:
                return {
            ...
            temp_count: Dict[Tuple[str, str], int] = {}
            temp_bytes: Dict[Tuple[str, str], int] = {}
            for (_sid, stream, reason), cnt in self._drop_count.items():
                k = (stream, reason)
                temp_count[k] = temp_count.get(k, 0) + int(cnt or 0)
                temp_bytes[k] = temp_bytes.get(k, 0) + int(self._drop_bytes.get((_sid, stream, reason), 0) or 0)
            agg_count = temp_count
            agg_bytes = temp_bytes

        total_count = int(getattr(self, "_chunks_dropped", 0) or 0)
        total_bytes = int(getattr(self, "_bytes_dropped", 0) or 0)

        by_stream: Dict[str, Dict[str, int]] = {}
        by_reason: Dict[str, Dict[str, int]] = {}

        for (stream, reason), cnt in agg_count.items():
            b = int(agg_bytes.get((stream, reason), 0) or 0)

            by_stream.setdefault(stream, {"count": 0, "bytes": 0})
            by_stream[stream]["count"] += int(cnt or 0)
            by_stream[stream]["bytes"] += b

            by_reason.setdefault(reason, {"count": 0, "bytes": 0})
            by_reason[reason]["count"] += int(cnt or 0)
            by_reason[reason]["bytes"] += b

        # Top-N reasons by bytes
        items = sorted(by_reason.items(), key=lambda kv: kv[1].get("bytes", 0), reverse=True)
        top = dict(items[: max(0, int(top_n or 0))]) if top_n else dict(items)
        other_items = items[max(0, int(top_n or 0)) :] if top_n else []

        if other_items:
            other_count = sum(int(v.get("count", 0) or 0) for _k, v in other_items)
            other_bytes = sum(int(v.get("bytes", 0) or 0) for _k, v in other_items)
            if other_count or other_bytes:
                top["__other__"] = {"count": other_count, "bytes": other_bytes}

        return {
            "total_drops": {"count": total_count, "bytes": total_bytes},
            "drop_by_why": top,
            "drop_by_stream": by_stream,
        }
    ...
    def _account_forward_result(self, *, sid: int, stream: str, sent: int, dropped: int, reason: str) -> None:
        ...
        if dropped:
            ...
            self._drop_count_agg[(stream, reason)] += 1
            self._drop_bytes_agg[(stream, reason)] += dropped
    ...
    def _account_local_drop(self, *, sid: int, stream: str, dropped: int, reason: str, chunks: int = 1) -> None:
        ...
        if chunks > 0:
            ...
            self._drop_count_agg[(stream, reason)] += chunks
    ...
    def _ensure_startup_buffer_state(self, sid: int) -> Dict[str, Any]:
        ...
        if st is not None:
            return st
        st = {
            "limit": {"c2s": int(self.c2s_buffer_bytes), "s2c": int(self.s2c_buffer_bytes)},
            "bytes": {"c2s": 0, "s2c": 0},
            "buf": {"c2s": deque(), "s2c": deque()},
            "active": {"c2s": (self.c2s_buffer_bytes > 0), "s2c": (self.s2c_buffer_bytes > 0)},
        }
    ...
    async def _flush_startup_buffers_on_close(self, *, flow: FlowKey, reason: str) -> None:
        ...
        try:
            buffered = int(sum(int(v or 0) for v in (st.get("bytes", {}) or {}).values()))
        except Exception:
            buffered = 0
        if buffered <= 0:
            self._startup_buffers.pop(sid, None)
        ...
        try:
            leftover = int(sum(int(v or 0) for v in (st.get("bytes", {}) or {}).values()))
        except Exception:
            leftover = 0
        if leftover > 0:
            ...
            self._startup_buffers.pop(sid, None)
    async def _flush_startup_buffer_on_half_close(self, *, flow: FlowKey, direction: str, reason: str) -> None:
        """Try hard to flush any startup-buffered bytes for one direction before we half-close outputs.

        Half-close events are delivered through the forward queue to preserve ordering relative to payload chunks.
        """
        if self.startup_buffer_bytes <= 0:
            return
        sid = flow.session_id
        ...
        # Small grace to allow last data chunks already queued to flush.
        try:
            close_grace_ms = int(get_path(self.cfg, "runtime.close_grace_ms", 100) or 0)
        except Exception:
            close_grace_ms = 100
        ...
        # Discard any leftover buffered bytes for this direction to avoid later close-time drops.
        q2 = st["buf"].get(direction)
        if q2:
            stream = self.outputs.map_stream(direction)
            dropped_bytes = 0
            dropped_chunks = 0
            while q2:
                ...
                dropped_chunks += 1
            if dropped_bytes:
                self._account_local_drop(
                    sid=sid,
                    stream=stream,
                    dropped=dropped_bytes,
                    reason="startup_buffer_discard_on_half_close",
                    chunks=dropped_chunks,
                )

        try:
            st["bytes"][direction] = 0
            st["active"][direction] = False
        except Exception:
            pass
    ...
    async def _consume_events(self) -> None:
        while True:
            ...
            if ev.kind == "open":
                ...
                # Requested: always DEBUG, including ack_stall (tcp_half_close is handled via forward queue to preserve ordering)
                await self.router.emit(
                    cat="flow",
                    event="tcp_note",
                    level="debug",
                    payload={"flow": ev.flow.to_dict(), **(ev.data if isinstance(ev.data, dict) else {})},
                    cp=True,
                )
    ...
    async def _consume_forward(self) -> None:
        while True:
            ...
            # Kick outputs as early as possible to avoid a race where forwardQ is processed
            # before the session-open event creates output targets (startup_buffer_full at connect burst).
            await self.outputs.ensure_session(ch.flow)

            kind = getattr(ch, "kind", "data")
            if kind == "half_close":
                direction = str(ch.direction).lower()
                stream = self.outputs.map_stream(direction)
                try:
                    r = (ch.meta or {}).get("reason")
                except Exception:
                    r = None
                reason = str(r or "fin")

                await self._flush_startup_buffer_on_half_close(flow=ch.flow, direction=direction, reason=reason)
                await self.outputs.close_stream(ch.flow, stream=stream, reason=reason, cause="input_half_close")
                await self.router.emit(
                    cat="session",
                    event="tcp_half_close",
                    level="info",
                    payload={"flow": ch.flow.to_dict(), "direction": direction, "stream": stream, "reason": reason},
                    cp=True,
                )
                continue
            ...
            if self.startup_buffer_bytes > 0:
                ...
                if st["active"].get(ch.direction, False):
                    ...
                    if ready:
                        ...
                        limit = int(st["limit"].get(ch.direction, 0) or 0)
                        used = int(st["bytes"].get(ch.direction, 0) or 0)
                        ...
                        if len(ch.data) <= remaining:
                            ...
                            st["bytes"][ch.direction] += len(ch.data)
                        else:
                            st["buf"][ch.direction].append(ch.data[:remaining])
                            st["bytes"][ch.direction] += remaining
