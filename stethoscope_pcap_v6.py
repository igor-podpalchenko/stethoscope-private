...
class PcapSink:
    ...
    def from_cfg(cfg: Dict[str, Any], *, local_ip: str, remote_ip: str, remote_port: int, idle_close_sec: float, log: logging.Logger) -> "PcapSink":
        ...
        link = pc.get("linktype", pc.get("dlt", None))
        if link is None:
            cap = get_path(cfg, "io.input.capture", {}) or {}
            link = cap.get("linktype", cap.get("dlt", None))
        ...
        return PcapSink(
            ...
            fin_close_grace_sec=float(fin_grace or 0.0),
            linktype=link,
            log=log,
        )
...
class OutputManager:
    def __init__(self, *, cfg: Dict[str, Any], router: EventRouter) -> None:
        ...
        if self.connector_enabled:
            if not self.conn_host:
                raise SystemExit("connector enabled, but io.output.remote-host.host is missing/empty")
            if self.conn_req_port <= 0 or self.conn_resp_port <= 0:
                raise SystemExit("connector enabled, but io.output.remote-host.requests_port/responses_port must be > 0")
