...
type Service struct {
	...
	// Per-direction startup buffering (backward compatible with legacy io.input.capture.buffer_bytes)
	startupBufferBytes    int // max(c2s,s2c) for convenience checks
	startupBufferC2SBytes int
	startupBufferS2CBytes int
...
func NewService(cfg Config, log *Logger) (*Service, error) {
	...
	baseStartupBuf := ClampInt(capCfg["buffer_bytes"], defaultStartupBufferBytes, 0, maxStartupBufferBytes)
	c2sStartupBuf := ClampInt(capCfg["c2s_buffer_bytes"], baseStartupBuf, 0, maxStartupBufferBytes)
	s2cStartupBuf := ClampInt(capCfg["s2c_buffer_bytes"], baseStartupBuf, 0, maxStartupBufferBytes)
	startupBufMax := c2sStartupBuf
	if s2cStartupBuf > startupBufMax {
		startupBufMax = s2cStartupBuf
	}
	...
	s := &Service{
		...
		ackStall:              ackStall,
		startupBufferBytes:    startupBufMax,
		startupBufferC2SBytes: c2sStartupBuf,
		startupBufferS2CBytes: s2cStartupBuf,
		startupBuffers:        map[int]*StartupBuffer{},
...
func (s *Service) ensureStartupBufferState(sid int) *StartupBuffer {
	...
	sb := &StartupBuffer{
		limit: map[string]int{"c2s": s.startupBufferC2SBytes, "s2c": s.startupBufferS2CBytes},
		bytes: map[string]int{"c2s": 0, "s2c": 0},
		buf:   map[string][][]byte{"c2s": [][]byte{}, "s2c": [][]byte{}},
		active: map[string]bool{
			"c2s": s.startupBufferC2SBytes > 0,
			"s2c": s.startupBufferS2CBytes > 0,
		},
	}
...
func (s *Service) flushStartupBuffersOnClose(flow FlowKey, reason string) {
	...
	buffered := sb.bytes["c2s"] + sb.bytes["s2c"]
	s.startupMu.Unlock()
	...
	s.startupMu.Lock()
	leftover := sb.bytes["c2s"] + sb.bytes["s2c"]
	s.startupMu.Unlock()
